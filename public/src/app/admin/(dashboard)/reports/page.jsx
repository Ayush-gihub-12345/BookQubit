...
export default function AdminReportsPage() {
  ...
  const load = () => {
    setLoading(true);
    fetch(`/api/admin/reports?page=${page}`).then((r) => r.json())
  ...
  const toggle = async (id, resolved) => {
    await fetch("/api/admin/reports", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, resolved }) });
    load();
  };
  const remove = async (id) => {
    if (!confirm("Delete this report?")) return;
    await fetch("/api/admin/reports", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    load();
  };

  return (
    <div>
      <h1 className="flex items-center gap-2 text-2xl font-bold text-white"><Icon name="shieldCheck" size={20} /> Reports</h1>
      <p className="text-muted mt-1 text-sm">{total.toLocaleString()} reader-submitted corrections — unresolved shown first</p>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm">
                {r.book_slug ? (
                  ...
                  <span className="text-muted font-semibold">General feedback</span>
                )}
