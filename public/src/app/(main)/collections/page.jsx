...
export default async function CollectionsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-slate-500">Themed reading journeys, curated for you</p>
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {f.collections.map((c) => (
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">{c.count} {c.count === 1 ? "book" : "books"}</p>
            <p className="mt-4 text-sm font-semibold text-brand-600">Explore →</p>
          </Link>
        ))}
      </div>
      {!f.collections.length && <p className="py-20 text-center text-slate-500">No collections yet.</p>}
    </div>
  );
}
