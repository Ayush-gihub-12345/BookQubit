...
export async function generateMetadata() {
  ...
  return {
    ...
    alternates: { canonical: `/${lang}/leaderboard` },
  };
}
...
export default async function LeaderboardPage({ searchParams }) {
  ...
  const withParam = (patch) => {
    ...
    return `/${lang}/leaderboard?${params.toString()}`;
  };
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_260px]">
        ...
        {mode === "activity" && (
          <aside className="card h-fit space-y-5 p-5 hover:!translate-y-0">
            ...
            <form action={`/${lang}/leaderboard`} className="space-y-4">
              ...
              {(year || minBooks || genre) && (
                <Link href={`/${lang}/leaderboard`} className="text-muted block text-center text-xs hover:text-brand-600">{_("clearFiltersLink")}</Link>
              )}
            </form>
          </aside>
        )}
