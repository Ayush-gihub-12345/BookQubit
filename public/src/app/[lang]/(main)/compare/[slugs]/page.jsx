...
export async function generateMetadata({ params }) {
  const { slugs: slugParam, lang } = await params;
  ...
  return {
    ...
    alternates: { canonical: `/${lang}/compare/${slugParam}` },
  };
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <nav className="text-muted mb-6 text-sm">
        <Link href={`/${lang}/compare`} className="hover:text-brand-600">{_("compareCrumb")}</Link>
      ...
      <div className="mt-8 grid gap-6" style={gridStyle}>
        {books.map((b) => (
          <div key={b.slug} className="card p-5 text-center hover:!translate-y-0">
            <Link href={`/${lang}/books/${encodeURIComponent(b.slug)}`}>
            ...
            <Link href={`/${lang}/books/${encodeURIComponent(b.slug)}`} className="mt-3 block font-bold hover:text-brand-600">
      ...
      <div className="mt-10 text-center">
        <Link href={`/${lang}/compare`} className="btn-ghost inline-flex">
