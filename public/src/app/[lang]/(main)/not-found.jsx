  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16 text-center">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-3">
        <Link href={`/${lang}`} className="btn-primary">Back to Home</Link>
        <Link href={`/${lang}/books`} className="btn-ghost">Browse All Books</Link>
      </div>
      ...
      {popular.length > 0 && (
        <div className="mt-14 text-left">
          ...
          <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-6">
            {popular.map((b) => <BookCard key={b.id} book={b} hrefBase={`/${lang}/books`} />)}
          </div>
        </div>
      )}
    </div>
