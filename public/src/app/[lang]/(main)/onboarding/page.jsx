  ...
  if (!started) {
    return (
      <div className="mx-auto flex min-h-[75vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        ...
        <Link href={`/${lang}/account`} className="text-muted mt-4 text-xs hover:text-brand-600">Skip setup for now</Link>
      </div>
    );
  }
            ...
            {recs.length > 0 && (
              <>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  {recs.map((b) => (
                    <Link key={b.slug} href={`/${lang}/books/${encodeURIComponent(b.slug)}`} className="card group overflow-hidden text-left">
