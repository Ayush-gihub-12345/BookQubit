...
export default function AuthButton() {
  ...
  const QUICK_LINKS = [
    { href: "/liked", icon: "heart", label: t("navLikedBooks") },
  ...
  if (!user) {
    return (
      <Link
        href="/login"
        className="btn-ghost hidden px-4 py-2 text-sm sm:inline-flex"
      >
        {t("signIn")}
      </Link>
    );
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        {user.photoURL ? (
          ...
          <img
            src={user.photoURL}
            alt=""
            className="h-9 w-9 rounded-full ring-2 ring-brand-500/50"
          />
        ) : (
      ...
      {open && (
        <div className="absolute right-0 top-12 z-[60] w-56 overflow-hidden rounded-xl border border-line bg-surface shadow-xl">
          ...
          >
            <Icon name="user" size={15} className="text-muted" /> {t("account")}
          </Link>
          {QUICK_LINKS.map((l) => (
            <Link
              key={l.href}
